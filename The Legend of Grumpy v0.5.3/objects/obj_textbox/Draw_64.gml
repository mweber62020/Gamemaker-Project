...
if (isChoice)
{
	// Prompt text in upper portion of box
	draw_set_halign(fa_center);
	draw_set_valign(fa_top);
	draw_text_ext_color(gui_w / 2, box_y + 12, textToShow, line_height, gui_w - (margin * 2) - 40, c_white, c_white, c_white, c_white, image_alpha);

	// Choices listed vertically in lower portion, cursor marks selected
	draw_set_halign(fa_left);
	draw_set_valign(fa_top);
	var choice_x = gui_w / 2 - 30;
	var choice_start_y = box_y + 68;
	for (var i = 0; i < array_length(choiceOptions); i++)
	{
		var prefix = (i == choiceIndex) ? "> " : "  ";
		draw_text_color(choice_x, choice_start_y + (i * (line_height + 4)), prefix + choiceOptions[i], c_white, c_white, c_white, c_white, image_alpha);
	}
}
else
{
